...
function Case({CaseColor, Image, FeaturesTitle, Title, FriendlyURL}: CaseType) {
        ...
        <div className="case" style={{background: `#${CaseColor}`}}>
            ...
            <a href={caseUrl} target="_blank" rel="noreferrer">
                <div className="case__info" style={{background: `#${CaseColor}`}}>
                    <h4 className={`case__title ${whiteColors.includes(CaseColor) ? 'case__title--black' : ''}`}>{Title}</h4>
                    <p className={`case__text ${whiteColors.includes(CaseColor) ? 'case__text--black' : ''}`}>{FeaturesTitle}</p>
                </div>
            </a>
        </div>
    );
